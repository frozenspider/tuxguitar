...
public class MidiSongReader extends MidiFileFormat implements TGSongReader {
	...
	private void parseMessage(int trackNumber,long tick,MidiMessage message){
		...
		//TRACK NAME
			...
			parseTrackName(trackNumber, message.getData());
		}
		//TIME SIGNATURE
	...
	private void parseTrackName(int trackNumber, byte[] data) {
		TGTrack track = getTrack(trackNumber);
		track.setName(new String(data, Charset.forName("UTF-8")));
	}
	...
	private void checkTracks(TGSongManager songManager){
		...
		while(it.hasNext()){
			...
			if (trackChannel == null && track.countMeasures() == 0 && track.getLyrics().isEmpty()) {
				// This track is empty and meaningless, most likely created while importing
				// MIDI with a named empty metadata-only track, like the one generated by Guitar Pro
				it.remove();
			} else {
